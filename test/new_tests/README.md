...
Each value translated to a `dub build`, `dub test`, or, `dub run` invocation.
...
While the runner tries to isolate each test by passing a specific `DUB_HOME` directory to each one in order to avoid concurrent build of the same (named) package this is not always possible.

For example, if three tests depend on the same library dependency in `extra/` those could not be run at the same time.
...
This setting controls the default behavior of interpreting the exit status of the program as success or failure.
...
`[FAIL]:` and `[SKIP]:` use the remaining portion of the output line to tell the user why the test was skipped so try to print something meaningful.
